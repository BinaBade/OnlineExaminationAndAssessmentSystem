    ...
    $.feedbackTab = function (p) {
        ...
        var feedbackTab = {
            ...
            DisplaySingleQuestionAnswers: function (examID) {
                ...
                $.ajax({
                    type: "POST",
                    url: feedbackTab.config.baseURL + "GetMockTestQuestions",
                    data: param,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
            ...
            GetExamList: function () {
                debugger;
                var webMethod = "http://localhost:49613/Exam/GetAllMockTest";
            ...
            GetMyResult: function (examID) {
                ...
                $.ajax({
                    type: "POST",
                    url: feedbackTab.config.baseURL + "GetMyExamAnswers",
                    data: param,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data) {
                        ...
                        $.each(data, function (i, item) {
                            ...
                            if (item.userSelectedAnswerID == item.objectiveAns_ID && item.status == 1) {
                                score = score + 1;
                            }
                            if (n != item.objectiveQue_ID) {
                                myhtml += '</div>';
                                ...
                                if (i == 0) {
                                    ...
                                    myhtml += '<div id="CheckQueDiv' + num + '" class="thisis' + num + '">'
                                }
                                else {
                                    myhtml += '<div id="CheckQueDiv' + num + '">'
                                }
                                myhtml += '<div class="row-fluid"><b>' + num + '. ' + item.objectiveQue + '</b></div>';
                            }
                            if (item.userSelectedAnswerID == item.objectiveAns_ID&&item.status==0) {
                                myhtml += '<div id="AnswerDivID" class="AnswerDivs"><label class="ansdivmistake" name="AnsName' + num + '" id="AnsClass' + num + '">' + item.objectiveAns + '</label></div>';
                            }
                            else {
                                if(item.status==1) {
                                    myhtml += '<div id="AnswerDivID" class="AnswerDivs"><label class="ansdivcorrect" name="AnsName' + num + '" id="AnsClass' + num + '">' + item.objectiveAns + '</label></div>';
                                }
                            else {
                                myhtml += '<div id="AnswerDivID" class="AnswerDivs"><label class="ansdiv" name="AnsName' + num + '" id="AnsClass' + item.objectiveAns_ID + '">' + item.objectiveAns + '</label></div>';
                            }
                            } 
                            ...
                            if (i == ((num * 4) - 1)) {
                                if (i == 3) {
                                        myhtml += '<button type="button" class="btnnext1 btn-block span3" style="padding-left:200px;" id="' + num + '">Next</button>';
                                }
                                else if (i == (end - 1)) {
                                    myhtml += '<button type="button" class="btnprevious1 btn-block span3" style="padding-right:200px;" id="' + num + '">Previous</button>';
                                }
                                else {
                                    myhtml += '<button type="button" class="btnprevious1 btn-block span3" style="padding-right:200px;" id="' + num + '">Previous</button>';
                                    myhtml += '<button type="button" class="btnnext1 btn-block span3" style="padding-left:200px;" id="' + num + '">Next</button>';
                        ...
                        myhtml += '</div>';
                        document.getElementById("showcorretanswersdiv").innerHTML = myhtml;
                        $("#spanscore").text("Your Score: " + score);
                        //var examID = $("#spanexamid").text();
                        //var usergivenans = feedbackTab.GetUserGivenAns(examID);
                        //debugger;
                        //$.each(usergivenans, function (i, item) {
                        //    debugger;
                        //    if (item.userSelectedAnswerID==item.objectiveAns_ID && item.status==0) {
                        //        $('#AnswerDivID').find('#AnsClass' + item.objectiveAns_ID).css("background", "red");
                        //    }
                        //    else {
                        //        return null;
                        //    }
                        ...
                        $(".btnnext1").on("click", function () {
                            debugger;
                            var nextid = parseInt($(this).attr("id"));
                            $("#CheckQueDiv" + nextid).hide();
                            nextid = nextid + 1;
                            $("#CheckQueDiv" + nextid).show();

                        });

                        $(".btnprevious1").on("click", function () {
                            debugger;
                            var nextid = parseInt($(this).attr('id'));
                            $("#CheckQueDiv" + nextid).hide();
                            nextid = nextid - 1;
            ...
            //GetUserGivenAns: function (examID) {
            //    debugger;
            //    var param = JSON2.stringify({
            //        examID: examID,
            ...
            init: function () {
                debugger;
                 ...
                 $("#gobackdiv").hide();
                    ...
                    $("#btngoback").on('click', function () {
                        var r = confirm("Do you want to Exit and Cancel Exam?");
                        if (r == true) {
                            $("#answerdiv").hide();
                 $("#gobackdiv").hide();
                    ...
                    $("#scorediv").hide();
                    $("#ExamDivList").show();
                    document.getElementById("showcorretanswersdiv").innerHTML = "";
                ...
                $("#btnshowcorrectans").on('click', function () {
                    debugger;
                    var examID = $("#spanexamid").text();
                    feedbackTab.GetMyResult(examID);
                });
